      ...
      <ul className="w-full ">
         {navData.map((item, idx) => (
            <li key={idx} className="border-b border-x first:border-t border-gray-700 cursor-pointer">
               {/* menu item */}
               <div className="flex justify-between items-center" onClick={() => handleToggle(idx)}>
                  <Link to={item.direct_path} className={`${openMenu === idx && "font-semibold transition-all opacity-50"} p-3 uppercase text-sm`}>{item.title}</Link>
                  {item.submenu && (
                     ...
                     <ul className="mt-3">
                        {item.submenu.map((subItem, i) => (
                           <li key={i} className="border-t last:border-b-0 border-gray-700 px-4 py-2.5 hover:bg-gray-300 ">
                              <Link to={subItem.path} className='uppercase text-sm'>{subItem.name}</Link>
                           </li>
                        ))}
                     </ul>
                  )}
