...
const NewArrival = () => {
   ...
   return (
      <div className='grid grid-cols-2 md:grid-cols-5 gap-x-2 gap-y-20 py-4 px-3'>
         {
            products.map((p) => (
               <Link id='productCard' to={`/product_details/${p.id}`} key={p.id} className='h-56 md:h-64 transition-all relative'>
                  ...
                  <div id='hoverElements' className='space-x-5'>
                     ...
                     <p to={`/product_details/${p.id}`} className='w-8 h-8 flex justify-center items-center rounded-sm text-white cursor-pointer bg-gray-600'><IoEyeOutline className='size-5 hover:opacity-50 transition-all' /></p>
                  </div>
                  <div>
